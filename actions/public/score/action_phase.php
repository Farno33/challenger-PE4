	...
	for ($ligne = 1; $ligne <= $lignes; $ligne++) {	
		for ($etape = 1; $etape <= $etapes + ($hasPetiteFinale ? 1 : 0); $etape++) {
			...
			if ($isMatch) {
				...
				$line = '<td onclick="window.location.href = \''.url('public/score/phase_'.$id_phase.'?m='.$numero.'&'.rand().'#m'.$numero, false, false).'\'" class="match'.($ecole === $a || $ecole === $b ? ' match-highlight' : '').($etape > $etapes ? ' petite-finale' : '').'">'.
...
$pow = floor(log($nbEquipes, 2));
$nbEquipesFinales = pow(2, $pow);
...
require DIR.'templates/public/score/phase_'.$phase['type'].'.php';
