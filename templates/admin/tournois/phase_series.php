...
?>
			...
			<div id="modal-match" class="modal big-modal">
					<form method="post" action="<?php url((empty($vpTournois) ? 'admin/module/tournois/' : 'centralien/vptournoi/').'phase_'.$id_phase.(!empty($_GET['p']) ? '?p='.$_GET['p'].($_GET['p'] == 'matchs' && !empty($select['id']) ? '#m'.$select['id'] : '#c'.$_GET['a']) : '#c'.$_GET['a'])); ?>">
						<fieldset>
