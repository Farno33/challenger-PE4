...
?>
				...
				<nav class="subnav">
					<form method="post" action="<?php url('admin/module/competition/extract'); ?>">
						<h2>
							Détails du tournoi<br /> 
							<?php echo stripslashes($sport['sport']).' '.printSexe($sport['sexe']); ?>
							<?php if (empty($vpTournois)) { ?> <input type="submit" value="CQL" /> <?php } ?>
						</h2>

						<input type="hidden" name="cql" value='<?php echo isset($_GET['p']) && 
							$_GET['p'] == 'concurrents' ? $cql_concurrents : $cql; ?>' />
					</form>
					

					<ul>
						<li><a href="?p=phases">Phases</a></li>
						<li><a href="?p=concurrents">Concurrents</a></li>
						<li><a href="?p=podium">Podium</a></li>
						<li><a href="?p=infos">Infos</a></li>
					</ul>
				</nav>
				...
				<table class="table-small">
					...
					<tbody>
						...
						<tr class="form clickme" onclick="window.location.href = '<?php url((empty($vpTournois) ? 'admin/module/tournois/' : 'centralien/vptournoi/').($sport['tournoi_initie'] ? 'phase_'.$phase['id'] : $id_sport .'?edit='.$phase['id'])); ?>';">
							<td class="content"><?php echo $phase['nom']; ?></td>
				...
				<form method="post">
					<center>
					<?php  ?><br /><input class = "delete" type="submit" name="delete_tournoi" value="Supprimer le tournoi" /><?php ?>
					</center>
				</form>
				...
				<?php if (!$sport['tournoi_initie']) { ?><center><button class="button" onclick="Array.from(document.getElementById('tab_concurents').children).forEach((e)=>{e.lastElementChild.firstElementChild.click();});">Inverser la sélection</button></center> <?php } ?>
				...
				$('input[type=checkbox]').on('change', function() {
					$.ajax({
						url: "<?php url((empty($vpTournois) ? 'admin/module/tournois/' : 'centralien/vptournoi/').$id_sport.'?ajax'); ?>",
						method: "POST",
						data: {
				...
				<?php } } else if ($_GET['p'] == 'infos') {?>
					<form method="post" class="form-table" >
						<div>
							<fieldset>
								<h3>Informations</h3>
								<div style="background-color: white">
									<textarea id="form-infos" name="infos"><?php echo stripslashes($sport['infos']); ?></textarea>
								</div>
								<a href="https://markdownlivepreview.com/" target="_blank">Si vous souhaitez être plus précis sur le tests de la syntaxe</a>
								<center>
									<input type="submit" class="success" value="Enregistrer les modifications" />
								</center>
							</fieldset>
						</div>
					...
					<script>
        				...
        				simplemdeCSS.setAttribute("rel", "stylesheet")
        				simplemdeCSS.setAttribute("type", "text/css")
        				simplemdeCSS.setAttribute("href", 'https://cdn.jsdelivr.net/simplemde/latest/simplemde.min.css')
						var simplemdeJS=document.createElement("script")
        				simplemdeJS.setAttribute("type", "text/javascript")
        				simplemdeJS.setAttribute("src", 'https://cdn.jsdelivr.net/simplemde/latest/simplemde.min.js')
						
						simplemdeJS.onload = function() {
							var simplemde = new SimpleMDE({ 
								element: document.getElementById("form-infos"), 
								autofocus: true, 
								forceSync: true, 
								placeholder: 'Informations sur le tournois au format markdown (au sens strict, les bidouilles html ne passeront pas sur l\'app) (plus d\'infos en cliquant sur le point d\'interogation dans la barre au dessus)\nAttention l\'apperçu n\'est pas totalement fidèle (principalement sur les titres, les couleurs sont un peu fantaisistes) vous pouvez utiliser le lien en dessous de l\'éditeur pour plus de fidélité\n\nCe texte sert principalmement dans l\'application. Attention ce n\'y est pas fréquemment mis à jours (journalier) donc n\'utilisez pas ça comme canal de communication, mais plus un rappel rapide du livret du participant'
							});
						}
						
						document.documentElement.appendChild(simplemdeCSS)
						document.documentElement.appendChild(simplemdeJS)
					</script>
				<?php } ?>
<?php 
