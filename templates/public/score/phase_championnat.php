...
?>
						...
						<tr>
							...
							foreach ($concurrents as $id_j => $concurrent_j) { ?>
								...
								<td style="background:<?php echo !empty($edit) && ($edit == $id_i.'_'.$id_j || $edit == $id_j.'_'.$id_i) ? '#BFB' : ($match == null ? '#FD9' : ($match['gagne'] == null ? '#DDF' : 'auto')); ?>" 
									<?php if ($phase['nb_precedentes_ouvertes'] == 0) { ?>class="clickme" onclick="window.location.href = '<?php url('score/tournois/phase_'.$phase['id'].'?a='.$id_i.'&b='.$id_j.'&t='.(!empty($_GET['t']) ? '1' : '').'&'.rand().'#c'.$id_i); ?>';" <?php } ?>>
			...
			<table>
				...
				<tbody>
					...
					<tr style="height:49% !important" class="form<?php if ($phase['nb_precedentes_ouvertes'] == 0) { ?> clickme match-<?php echo $match['id']; ?>" data-id="<?php echo $match['id']; ?>" onclick="window.location.href = '<?php url('score/tournois/phase_'.$phase['id'].'?p=matchs&a='.$match['id_concurrent_a'].'&b='.$match['id_concurrent_b'].'&'.rand().'#m'.$match['id']); ?>';<?php } ?>">
			...
			<div id="modal-match" class="modal big-modal">
					<form method="post" action="<?php url('score/tournois/phase_'.$id_phase.'?t='.(!empty($_GET['t']) ? '1' : '').(!empty($_GET['p']) ? '&p='.$_GET['p'].($_GET['p'] == 'matchs' && !empty($select['id']) ? '#m'.$select['id'] : '#c'.$_GET['a']) : '#c'.$_GET['a'])); ?>">
						<fieldset>
							<legend>Edition d'un match</legend>
